...
class TestFormatter:
    ...
    def init_test(self):
        ...
        self.expected_subject_df = pd.DataFrame({
            "id": ["id1"],
            "accountGuid": ["random_acct_guid"],
            "projectGuid": ["random_project_guid"],
            "ageAtEnrollment": [""],
        ...
        self.expected_sample_output = {
            ...
            pd.DataFrame({
                ...
                'totalViableCellCount': ['', ''],
                'additionalPayload': ['', ''],
                'projectGuid': ['pg-sample', 'pg-sample']
            }),
            'labResults':
            pd.DataFrame({
                'id': ['labid1'],
    ...
    def test_reshape_custom_metadata(self, init_test):
        formatted_metadata_df = hpf.reshape_custom_metadata(
            self.subject_metadata).sort_index(axis=1)

        # check column names match
        assert formatted_metadata_df.columns.tolist(
        ) == self.expected_subject_df.columns.tolist(
        ), "Column names do not match"

        # check values match
        diff = self.expected_subject_df.compare(formatted_metadata_df,
                                                align_axis=1)
    ...
    def test_sample_to_df(self, init_test):
        ...
        # check column names match
        assert formatted_sample_df['metadata'].sort_index(
            axis=1).columns.tolist(
            ) == self.expected_sample_output['metadata'].sort_index(
                axis=1).columns.tolist(), "metadata Column names do not match"
        assert formatted_sample_df['specimens'].sort_index(
            axis=1).columns.tolist(
            ) == self.expected_sample_output['specimens'].sort_index(
                axis=1).columns.tolist(), "specimens Column names do not match"
        assert formatted_sample_df['labResults'].sort_index(
            axis=1).columns.tolist(
            ) == self.expected_sample_output['labResults'].sort_index(
                axis=1).columns.tolist(
                ), "labResults Column names do not match"
        assert formatted_sample_df['survey'].sort_index(axis=1).columns.tolist(
        ) == self.expected_sample_output['survey'].sort_index(
            axis=1).columns.tolist(), "survey Column names do not match"

        # check values match
        metadata_diff = self.expected_sample_output['metadata'].sort_index(
            axis=1).compare(formatted_sample_df['metadata'].sort_index(axis=1),
                            align_axis=1)
        assert metadata_diff.empty, f"metadata Dataframes are not equal: {metadata_diff}"

        specimens_diff = self.expected_sample_output['specimens'].compare(
            formatted_sample_df['specimens'], align_axis=1)
        assert specimens_diff.empty, f"specimens Dataframes are not equal: {specimens_diff}"

        labResults_diff = self.expected_sample_output['labResults'].compare(
            formatted_sample_df['labResults'], align_axis=1)
        assert labResults_diff.empty, f"labResults Dataframes are not equal: {labResults_diff}"

        survey_diff = self.expected_sample_output['survey'].sort_index(
            axis=1).compare(formatted_sample_df['survey'].sort_index(axis=1),
                            align_axis=1)
