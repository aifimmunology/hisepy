...
class TestReader:
    ...
    def test_validate_download_params(self):
        ...
        query_id = ['query_id1']
        query_dict = {'fileType': ['randomtype'], 'field1': ['value1']}
        assert hpru.validate_download_params(
    ...
    def test_fail_validate_download_params(self):
        ...
        with pytest.raises(Exception,
            ...
            hpru.validate_download_params(file_list=bad_file_ids,
        ...
        with pytest.raises(
            ...
            hpru.validate_download_params(file_list=None,
                                          query_id=None,
                                          query_dict=None)
    ...
    def test_log_downloaded_files_or_samples(self):
        ...
        hpru.log_project_download('file_id1', cwd)
    ...
    def test_log_replica_file_download(self, init_test):
        ...
        hpru.log_replica_file_download(self.descriptor_obj, 'file_id1', cwd)
        assert not os.path.exists(
            '.hisefilelog.rds'
        ), "this was not a replica file, so it should not be logged in this case"

        # now create a log file
        hpru.log_replica_file_download(self.descriptor_obj, 'file_id2', cwd)
        assert os.path.exists('.hisefilelog.rds'), "Failed to save log file"
    ...
    def test_convert_query_dict_to_mongo_query(self, init_test):
        ...
        with patch("hisepy.lookup.list_queryable_fields",
                   return_value=['field1']):
            mq = hpru.MongoQuery(self.query_dict)
        ...
        with patch("hisepy.lookup.list_queryable_fields",
            ...
            mq2 = hpru.MongoQuery(self.query_file_dict)
    ...
    def test_fail_validate_query_files_params(self, init_test):
        with pytest.raises(
                ...
                match=
                "One of file_ids, query_dict, or query_id must be a non-null"):
            hpru.validate_post_query_params(None, None, None)

        with pytest.raises(AssertionError,
                           match="You must only use 1 parameter"):
            hpru.validate_post_query_params(self.file_ids, self.query_id, None)

        with pytest.raises(
                AssertionError,
                match="You must pass a list of file ids to read_files"):
            hpru.validate_post_query_params(self.file_ids, None,
                                            self.query_dict)

    def test_validate_query_files_params(self, init_test):
        hpru.validate_post_query_params(self.file_ids, None, None)
        hpru.validate_post_query_params(None, self.query_id[0], None)
        hpru.validate_post_query_params(None, None, self.query_dict)
    ...
    def test_validate_samples_subjects_params(self, init_test):
        hpru.validate_samples_subjects_params(self.file_ids, None)
        hpru.validate_samples_subjects_params(None, self.query_dict)

    def test_fail_validate_samples_subjects_params(self, init_test):
        with pytest.raises(
                TypeError,
                match="Specify either `ids_list` or `query_dict`, but not both."
        ):
            hpru.validate_samples_subjects_params(None, None)
        with pytest.raises(
                TypeError,
                match="Specify either `ids_list` or `query_dict`, but not both."
        ):
