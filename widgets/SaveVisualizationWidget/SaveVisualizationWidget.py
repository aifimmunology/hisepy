...
class SaveVisualizationWidget:
    ...
    def __init__(self, name_space_magics=NamespaceMagics, interactive_shell=InteractiveShell):
        self.namespace = name_space_magics()
        ishl = interactive_shell.instance()
        self.namespace.shell = ishl # ipython.kernel.shell -- doing this with local var or it fails to create instance
        
        self.save_visualization_result = None
        self.read_files_result = None
        ...
        self.pf_options = [(i, i)for i in user_ns
                ...
                and type(user_ns[i]) is plotly.graph_objs.Figure]
        ...
        def on_save_button_click(evt):
            if self.study_space_dropdown.value is not None and len(self.title_text.value) >= SaveVisualizationWidget.min_title_length:
                ...
                if self.input_file_ids.value is not None:
                    self.read_files_result = hp.read_files(file_list=self.resolve_ids(self.input_file_ids.value))
