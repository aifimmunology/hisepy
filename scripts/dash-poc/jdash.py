...
cyto_dict_df = hp.read_files(file_list=file_list,
                             query_id=None,
                             query_dict=None,
                             to_df=True)
cyto_dict_df_merged = pd.merge(cyto_dict_df['labResults'],
                               cyto_dict_df['descriptors'],
                               left_on='sampleKitGuid',
                               right_on='sample.sampleKitGuid')
pop_stats_file = 'cache/B007/population-stats.csv'
file_df = pd.read_csv(pop_stats_file)
descriptive = cyto_dict_df_merged[[
    'subject.biologicalSex', 'subject.ethnicity', 'subject.race'
...
app.layout = html.Div([
    ...
    html.H2(className='my-3',
            children='''Cytometry: Population Stats File(s)'''),
    html.Div(
        className='row mb-5 rounded border border-light',
        children=[
            html.Div(
                className='col',
                children=[
                    html.H5(className='text-secondary',
                            children=['Descriptive Statistics']),
                    html.
                    P(className='text-muted',
                      children=[
                          'Descriptive statistics for the population count column'
                      ]),
        ...
        children=[
            html.Div(
                ...
                children=[
                    ...
                    P(className='text-muted',
                      children=[
                          'Table view of the flow cytometry population stats counts. The list of columns and the types of those columns in the schema.'
                      ]),
                    dash_table.DataTable(
                        id='table',
                        columns=[{
                            "name": i,
                            "id": i
                        } for i in cyto_dict_df_merged.columns],
    ...
    html.Div(className='row mb-5 rounded border border-light',
             children=[
                 html.Div(className='col-4',
                          children=[
                              ...
                              html.P(className='text-muted',
                                     children=['Change plot layout']),
                              dcc.Dropdown(id='bar-mode',
                                           options=[{
                                               'label': i,
                                               'value': i
                                           } for i in bar_mode_options],
                                           value='group'),
                          ]),
             ]),
    html.Div(className='row mb-5 rounded border border-light',
             children=[
                 html.Div(className='col',
                          children=[
                              ...
                              html.P(className='text-muted',
                                     children=[
                                         'Plot of Cytometry Population Stats'
                                     ]),
                              html.Div(className="form-control",
                                       children=[dcc.Graph(id='cyto-graph')])
                          ]),
             ]),
    html.Div(className='row mb-5 rounded border border-light',
             children=[
                 html.Div(className='col',
                          children=[
                              html.H5(className='text-secondary',
                                      children=['SPLOM']),
                              html.P(className='text-muted',
                                     children=['Multiple Dimension']),
                              html.Div(className="form-control",
                                       children=[dcc.Graph(id='cyto-splom')])
                          ]),
             ]),
...
@app.callback(Output('cyto-graph', 'figure'), Output('cyto-splom', 'figure'),
...
def update_graph(bar_mode_name):
    ...
    fig.update_layout(barmode=bar_mode_name,
                      legend_title_text='Subject ID',
                      autosize=True,
                      height=640,
                      yaxis=dict(title_text="Population Count", ),
                      xaxis=dict(title_text="Population", ))
    ...
    splom_fig.update_layout(legend_title_text='Subject ID',
                            autosize=True,
                            height=800,
                            yaxis=dict(title_text="Population Count", ),
                            xaxis=dict(title_text="Population", ))
