...
class SaveDashAppWidget:
    ...
    def __init__(self):
        ...
        def on_save_button_click(evt):
            ...
            ) >= SaveDashAppWidget.min_title_length:
                # enforce user selected a single image
                if len(self.image_filepath.value) > 1:
                    raise AssertionError("must choose a single PNG file")
                else:
                    image_fpath = str(
                        Path(self.image_filepath.value[0]).resolve())

                print(
                    str(Path(self.image_filepath.value[0]).resolve())
                )  # or str(Path(self.image_filepath.value).resolve()) - for full path
                self.save_dash_app_result = hp.save_dash_app(
                    app_filepath=str(self.app_filepath.value),
        ...
        self.grid[1, 0] = widgets.Label(
            SaveDashAppWidget.study_space_dropdown_description)
        self.grid[1, 1] = self.study_space_dropdown
        self.grid[1,
                  2] = widgets.Label(SaveDashAppWidget.title_text_description)
        self.grid[1, 3] = self.title_text
        self.grid[1, 4] = widgets.Label(
            SaveDashAppWidget.description_text_description)
        ...
        self.grid[2, 2] = widgets.Label(
            SaveDashAppWidget.additional_files_description)
        self.grid[2, 3] = self.additional_files
        self.grid[2, 4] = widgets.Label(
            SaveDashAppWidget.image_filepath_description)
        self.grid[2, 5] = self.image_filepath
        # file or sample ids
        self.grid[3, 0] = widgets.Label(
            SaveDashAppWidget.input_file_ids_description)
        self.grid[3, 1] = self.input_file_ids
        self.grid[3, 2] = widgets.Label(
            SaveDashAppWidget.input_sample_ids_description)
