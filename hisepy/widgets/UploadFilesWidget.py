...
class UploadFilesWidget:
    ...
    def __init__(self):
        ...
        self.grid[1, 0] = widgets.Label(
            UploadFilesWidget.study_space_dropdown_description)
        self.grid[1, 1] = self.study_space_dropdown
        self.grid[1,
                  2] = widgets.Label(UploadFilesWidget.title_text_description)
        self.grid[1, 3] = self.title_text

        # app files
        self.grid[2, 0] = widgets.Label(
            UploadFilesWidget.additional_files_description)
        self.grid[2, 1] = self.additional_files

        # file or sample ids
        self.grid[3, 0] = widgets.Label(
            UploadFilesWidget.input_file_ids_description)
        self.grid[3, 1] = self.input_file_ids
        self.grid[3, 2] = widgets.Label(
            UploadFilesWidget.input_sample_ids_description)
    ...
    def resolve_ids(self, ids_str):
        if ids_str:
            ids_list = list(
                map(lambda id: str.strip(id),
                    ids_str.replace('"', "").split(',')))
            return ids_list
        else:
            return []
