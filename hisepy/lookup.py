...
def lookup_queryable_fields(field_type='all'): 
    ...
    for cf in collection_fields: 
        ...
        all_fields_df = pd.concat([all_fields_df, fields_df], ignore_index=True)
        ...
        if cf == 'sample': 
            all_fields_df = pd.concat([all_fields_df,pd.DataFrame({'field':['bridgingControl'], 'field_type':['sample']})], ignore_index=True)
