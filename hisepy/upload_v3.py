...
DEFAULT_STORE_KEY = 'default_filestore'
...
class HiseUser:
    ...
    def __init__(self):
        ...
        user_resp = parse_hise_response(
            requests.get(user_url, headers=get_bearer_token_header()))
        for key, value in user_resp:
            setattr(self, key, value)


def set_default_store(store=None):
    if store is None:
        raise ValueError("No store was specified")
    if store not in valid_upload_stores:
        raise ValueError("Value for store must be in %s" %
                         (", ".join(valid_upload_stores)))
    set_ide_default(DEFAULT_STORE_KEY, store)
    print("Default filestore for this IDE set to %s" % store)


def set_default_project(project=None):
    if project is None:
        raise ValueError("No project was specified")
    user = HiseUser()
    if project not in [p['short_name'] for p in user.current_projects]:
        raise ValueError("Project %s not found in user's current projects" %
                         project)
    set_ide_default(DEFAULT_PROJECT_KEY, project)
    print("Default project for this IDE set to %s" % project)


def upload_files_v2(files: list,
                    ...
                    do_prompt: bool = True):
    """
    Uploads files to a store and records their provenance in HISE
    ...
    elif has_ide_default(DEFAULT_STORE_KEY):
        store = get_ide_default(DEFAULT_STORE_KEY)
    ...
    elif has_ide_default(DEFAULT_PROJECT_KEY):
        qargs["project"] = get_ide_default(DEFAULT_PROJECT_KEY)
