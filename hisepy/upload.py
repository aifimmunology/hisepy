...
class DashAppImg:
    ...
    def get_input_data(self):
        """ return files that are not manifest files or python scripts """

        # list all files within directories
        files_within_dir = []
        for d in self.directories:
            files_within_dir.append(cu.list_all_filepaths(d))

        input_files = []
        for f in list(self.filepaths) + files_within_dir:
            if '.py' in f:  # TODO: what other files do we need to ignore
                continue
            else:
    ...
    def export_dash_image(self):
        ...
        input_files = self.get_input_data()
        app_file_list = list(set(input_files + manifest_files)) + [
        ...
        dash_flow_payload = {
            "dataSource":
            app_file_list[:-1],  # include everything, except tarball
            "dataFile":
            upload_resp['FileIds'][:-1],  # include everything, except tarball
